...
#include "ISXEQMedley.h"

// The mandatory pre-setup function.  Our name is "ISXEQMedley", and the class is ISXEQMedley.
...
// our instanced class.
ISXPreSetup("ISXEQMedley",ISXEQMedley);
...
bool ISXEQMedley::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQMedley Loaded");
return true;
}

// shutdown sequence
void ISXEQMedley::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQMedleyXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQMedleyXML);
...
void ISXEQMedley::ConnectServices()
{
...
void ISXEQMedley::RegisterCommands()
{
    // add any commands
...
void ISXEQMedley::RegisterAliases()
{
    // add any aliases
}

void ISXEQMedley::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQMedley::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQMedley::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQMedley::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQMedley::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQMedley::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQMedleyService)
    //        pISInterface->ShutdownService(this,hISXEQMedleyService);
}

void ISXEQMedley::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQMedleyXML
    ISXEQMedleyXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQMedleyXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQMedleyXML,"General"))
    {
